export declare const interceptors: {
    request: ((options: RequestInit) => Promise<RequestInit>) | null;
    response: ((output: {
        response: Response;
    }) => Promise<void>) | null;
};
/**
 * Make a get call to the backend
 * @param path - path
 * @param options - options to pass into the fetch
 */
export declare const get: <O>(path: string, options?: RequestInit) => Promise<{
    response: Response;
    data: O;
}>;
/**
 * Make a post call to the backend
 * @param path - path
 * @param body - body to pass in
 * @param options - options to pass into the fetch
 */
export declare const post: <O>(path: string, body: FormData | Record<string, unknown>, options?: RequestInit) => Promise<{
    response: Response;
    data: O;
}>;
